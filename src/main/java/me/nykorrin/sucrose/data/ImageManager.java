...
public class ImageManager implements Listener {
     ...
     */
    public void init() {
        Bukkit.getServer().getPluginManager().registerEvents(this, Sucrose.getPlugin(Sucrose.class));
        loadImages();
    }
    ...
    public void onMapInitEvent(MapInitializeEvent event) {
        if (hasImage(event.getMap().getId())) {
            ...
            view.addRenderer(new LogoRenderer(getImage(view.getId())));
    ...
    class CustomFile {
        ...
        private final Sucrose plugin = Sucrose.getPlugin(Sucrose.class);
        ...
        public void saveConfig() {
            ...
            try {
                ...
                plugin.getLogger().log(Level.SEVERE, "Could not save config to " + dataConfigFile, e);
            }
        }
